...
export async function PATCH(req: NextRequest) {
  ...
  const response = NextResponse.json({ success: true, user: Array.isArray(data) ? data[0] : data });
  if (body.name) {
    response.cookies.set('auth_name', body.name, { httpOnly: true, path: '/', maxAge: 60*60*24*7, sameSite: 'lax' });
  }
  return response;
}

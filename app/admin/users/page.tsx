  ...
  return (
    <div style={{ padding:32, fontFamily:"'DM Sans',sans-serif", maxWidth:960 }}>
      ...
      <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:28, flexWrap:'wrap', gap:16 }}>
        ...
        <div style={{ display:'flex', gap:10 }}>
          ...
          <button onClick={openAdd}
            style={{ display:'flex', alignItems:'center', gap:6, padding:'10px 20px', background:'linear-gradient(135deg,#2D3F8F,#1B2A6B)', color:'white', border:'none', borderRadius:12, fontSize:13, fontWeight:700, cursor:'pointer' }}>
            + Tambah User
          </button>
        </div>
      </div>
      ...
      <div style={{ background:'white', border:'1.5px solid #E8ECF8', borderRadius:16, overflow:'hidden' }}>
        {loading ? <div style={{ padding:48, textAlign:'center' }}><span style={{ width:24, height:24, border:'3px solid #E8ECF8', borderTopColor:'#2D3F8F', borderRadius:'50%', display:'inline-block', animation:'spin 0.7s linear infinite' }} /></div> : (
          <table style={{ width:'100%', borderCollapse:'collapse' }}>
            <thead><tr style={{ background:'#F8F9FF', borderBottom:'1.5px solid #E8ECF8' }}>
              {['User','Email','Role','Status','Bergabung','Aksi'].map(h => <th key={h} style={{ textAlign:'left', padding:'12px 16px', fontSize:11, fontWeight:700, color:'#94A3B8', textTransform:'uppercase', letterSpacing:'0.8px', whiteSpace:'nowrap' }}>{h}</th>)}
            </tr></thead>
            <tbody>
              {users.map(u => {
                ...
                return (
                  <tr key={u.id} className="user-row" style={{ borderBottom:'1px solid #F9FAFB', transition:'background 0.1s', opacity:u.active?1:0.55 }}>
                    ...
                    <td style={{ padding:'14px 16px' }}><span style={{ fontSize:11, fontWeight:700, padding:'4px 10px', borderRadius:20, background:m.bg, color:m.color }}>{m.icon} {m.label}</span></td>
                    <td style={{ padding:'14px 16px' }}><span style={{ fontSize:11, fontWeight:700, padding:'4px 10px', borderRadius:20, background:u.active?'#DCFCE7':'#FEE2E2', color:u.active?'#16A34A':'#DC2626' }}>{u.active?'● Active':'○ Inactive'}</span></td>
                    <td style={{ padding:'14px 16px', fontSize:12, color:'#94A3B8' }}>{new Date(u.created_at).toLocaleDateString('id-ID',{day:'numeric',month:'short',year:'numeric'})}</td>
                    <td style={{ padding:'14px 16px' }}>
      ...
      {/* Add/Edit Modal */}
      {showModal && (
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.5)', zIndex:1000, display:'flex', alignItems:'center', justifyContent:'center', padding:24 }} onClick={e => { if(e.target===e.currentTarget) setShowModal(false); }}>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
              ...
              <div>
                ...
                <select style={{ ...S.input, appearance:'none' }} value={form.role} onChange={e=>setForm({...form,role:e.target.value as Role})}>
                  {ROLES.map(r => <option key={r} value={r}>{ROLE_META[r].icon} {ROLE_META[r].label} — {ROLE_META[r].desc.split(' — ')[0]}</option>)}
                </select>
              </div>
      ...
      {/* Import CSV Modal */}
      {showImport && (
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.5)', zIndex:1000, display:'flex', alignItems:'center', justifyContent:'center', padding:24 }} onClick={e=>{if(e.target===e.currentTarget){setShowImport(false);}}}>
          <div style={{ background:'white', borderRadius:20, padding:32, width:'100%', maxWidth:520, animation:'fadeIn 0.2s ease', boxShadow:'0 24px 64px rgba(0,0,0,0.15)' }}>
            ...
            <div onClick={() => importRef.current?.click()}
              style={{ border:`2px dashed ${importFile?'#2D3F8F':'#C7D0F0'}`, borderRadius:12, padding:28, textAlign:'center', cursor:'pointer', background:importFile?'#F0F3FD':'#F8F9FF', marginBottom:16 }}>
              {importFile ? <div><div style={{fontSize:32,marginBottom:6}}>📄</div><p style={{fontWeight:600,color:'#2D3F8F',margin:'0 0 2px'}}>{importFile.name}</p><p style={{fontSize:12,color:'#9CA3AF',margin:0}}>{(importFile.size/1024).toFixed(1)} KB</p></div>
            ...
            <div style={{ display:'flex', gap:10 }}>
              ...
              <button onClick={handleImport} disabled={!importFile||importing} style={{ flex:2, padding:'12px', borderRadius:10, border:'none', background:(!importFile||importing)?'#9CA3AF':'linear-gradient(135deg,#2D3F8F,#1B2A6B)', color:'white', fontSize:14, fontWeight:700, cursor:(!importFile||importing)?'not-allowed':'pointer', display:'flex', alignItems:'center', justifyContent:'center', gap:8 }}>
                {importing?<><span style={{width:14,height:14,border:'2px solid rgba(255,255,255,0.3)',borderTopColor:'white',borderRadius:'50%',display:'inline-block',animation:'spin 0.7s linear infinite'}}/>Importing…</>:'📥 Import'}
              </button>
            </div>
          </div>
        </div>

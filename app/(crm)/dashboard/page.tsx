...
const STATUS_LABELS: Record<CustomerStatus, string> = {
  new: 'New', contacted: 'Contacted', negotiation: 'Negotiation', deal: 'Deal', lost: 'Lost',
};
...
export default async function DashboardPage() {
  // Safe fetch — never throw, always return empty array on error
  let all: Customer[] = [];
  let recent: Customer[] = [];
    ...
    <div className="p-8" style={{ fontFamily: "'DM Sans', sans-serif" }}>
      ...
      {fetchError && (
        <div className="mb-6 rounded-xl px-5 py-4" style={{ background: '#FEF2F2', border: '1.5px solid #FECACA' }}>
          ...
          <p className="text-xs mt-1" style={{ color: '#9CA3AF' }}>
            Periksa environment variables NEXT_PUBLIC_SUPABASE_URL + NEXT_PUBLIC_SUPABASE_ANON_KEY (minimal) atau SUPABASE_SERVICE_ROLE_KEY (recommended) di Vercel, lalu redeploy.
          </p>
        </div>
      )}
      ...
      {!((process.env.NEXT_PUBLIC_SUPABASE_URL ?? process.env.SUPABASE_URL) &&
        ...
        <div className="mb-6 rounded-xl px-5 py-4" style={{ background: '#FFFBEB', border: '1.5px solid #FDE68A' }}>
          ...
          <p className="text-xs mt-1" style={{ color: '#9CA3AF' }}>
            Tambahkan NEXT_PUBLIC_SUPABASE_URL dan NEXT_PUBLIC_SUPABASE_ANON_KEY (minimal) atau SUPABASE_SERVICE_ROLE_KEY (recommended) di Vercel → Settings → Environment Variables, lalu redeploy.
          </p>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-4 mb-8">
        {stats.map((s) => (
          <div key={s.label} className="rounded-2xl p-5 transition-shadow hover:shadow-md"
            ...
            <div className="w-10 h-10 rounded-xl flex items-center justify-center mb-3 text-lg" style={{ background: s.bg }}>
              {s.icon}
            </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        ...
        <div className="rounded-2xl p-6" style={{ background: '#fff', border: '1.5px solid #E8ECF8' }}>
          ...
          {all.length === 0 && !fetchError && (
            <p className="text-sm text-center py-4" style={{ color: '#9CA3AF' }}>Belum ada data.</p>
          )}
        </div>
        ...
        <div className="rounded-2xl p-6" style={{ background: '#fff', border: '1.5px solid #E8ECF8' }}>
          ...
          {recent.length === 0 ? (
            <p className="text-sm text-center py-8" style={{ color: '#9CA3AF' }}>
              {fetchError ? 'Gagal memuat data.' : 'No customers yet.'}
            </p>
          ) : (
            <div className="space-y-3">
              {recent.map((c) => {

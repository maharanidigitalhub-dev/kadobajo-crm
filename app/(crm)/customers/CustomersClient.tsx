...
export default function CustomersClient({
  ...
  const filtered = useMemo(() => {
    return customers.filter((c) => {
      ...
      const matchSource =
        !filterSource ||
        c.source_slug === filterSource ||
        c.utm_source === filterSource ||
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-[#8B7355] text-sm mt-0.5">
            {customers.length} total leads · {filtered.length} shown
          </p>
        </div>
                ...
                {countries.length > 0 && (
                  <>
                    ...
                    {countries.map((country) => (
                      <button key={country}
                        onClick={() => { exportByCountry(customers, country); setShowExportMenu(false); }}
            ...
            <button key={country}
              ...
              className={`px-3 py-1.5 rounded-full text-xs font-medium border transition-all ${
                filterCountry === country
                  ...
                  : 'bg-white text-[#4A2C1A] border-[#E8DFD0] hover:border-[#C4A35A]'
              }`}>
              🌍 {country} · {count}
            </button>
          ))}
          ...
          <table className="w-full">
            ...
            <tbody>
              {filtered.length === 0 ? (
                ...
                filtered.map((c, i) => (
                  <tr key={c.id}
                    className={`border-b border-[#F5F3EF] hover:bg-[#FDFCF9] transition-colors ${i === filtered.length - 1 ? 'border-0' : ''}`}>
          ...
          <form onSubmit={handleCreateManualCustomer}
            ...
            <div className="flex items-start justify-between gap-4">
              ...
              <button type="button" onClick={() => setShowAddModal(false)}
                className="text-[#8B7355] hover:text-[#2C1810] text-lg leading-none">×</button>
            </div>
            ...
            <div>
              ...
              <select value={manualForm.country}
            ...
            <div>
              ...
              <input type="text" value={manualForm.phone}

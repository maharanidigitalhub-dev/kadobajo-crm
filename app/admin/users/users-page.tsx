...
const VALID_ROLES: Role[] = ['superadmin', 'manager', 'admin', 'management'];
...
const BLANK = { name: '', email: '', password: '', role: 'admin' as Role };
...
export default function UsersPageClient() {
  ...
  async function handleSave() {
    ...
    try {
      if (editing) {
        ...
        if (!res.ok) { const d = await res.json(); setError(d.error ?? 'Gagal update'); return; }
        ...
        if (!res.ok) { const d = await res.json(); setError(d.error ?? 'Gagal membuat user'); return; }
  ...
  return (
    <div style={{ padding: 32, fontFamily: "'DM Sans',sans-serif", maxWidth: 960 }}>
      ...
      {success && (
        <div style={{ animation: 'fadeIn 0.2s ease', background: '#DCFCE7', border: '1px solid #86EFAC', borderRadius: 10, padding: '10px 16px', marginBottom: 16, fontSize: 13, color: '#16A34A', fontWeight: 600 }}>
          ✓ {success}
        </div>
      )}
        ...
        {VALID_ROLES.map(role => {
          ...
          return (
            <div key={role} style={{ background: 'white', border: '1.5px solid #E8ECF8', borderRadius: 12, padding: '14px 16px', display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <div>
                <p style={{ fontSize: 22, fontWeight: 800, color: m.color, margin: 0 }}>{count}</p>
      ...
      <div style={{ background: 'white', border: '1.5px solid #E8ECF8', borderRadius: 16, overflow: 'hidden' }}>
        {loading ? (
          ...
          <div style={{ padding: 48, textAlign: 'center', color: '#9CA3AF' }}>
            ...
            <p style={{ fontSize: 14, margin: 0 }}>Belum ada user. Klik "+ Tambah User" untuk mulai.</p>
          </div>
        ) : (
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {users.map(u => {
                const m = ROLE_META[u.role] ?? ROLE_META['management'];
          ...
          <div style={{ background: 'white', borderRadius: 20, padding: 32, width: '100%', maxWidth: 480, animation: 'fadeIn 0.2s ease', boxShadow: '0 24px 64px rgba(0,0,0,0.15)' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <div>
                ...
                <input style={{ ...S.input, ...(editing ? { background: '#F3F4F6', color: '#9CA3AF' } : {}) }} type="email" value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} disabled={!!editing} placeholder="email@kadobajo.com" />
              </div>
              <div>
                <label style={S.label}>{editing ? 'Password Baru (kosongkan jika tidak berubah)' : 'Password *'}</label>
                <input style={S.input} type="password" value={form.password} onChange={e => setForm({ ...form, password: e.target.value })} placeholder={editing ? 'Kosongkan jika tidak berubah' : 'Min 6 karakter'} />
              </div>
              <div>
                <label style={S.label}>Role *</label>
                <select style={{ ...S.input, appearance: 'none' as const }} value={form.role} onChange={e => setForm({ ...form, role: e.target.value as Role })}>
                  {VALID_ROLES.map(r => (

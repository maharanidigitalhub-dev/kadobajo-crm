...
type SaveStatus = 'idle' | 'saving' | 'saved' | 'pending_approval' | 'error';
...
function CMSInner() {
  ...
  // Get user role from cookie via API
  useEffect(() => {
  ...
  async function handleSave() {
    ...
    try {
      ...
      const newContent = { ...current, lp_overrides: overrides };

      // Admin role → submit approval request instead of saving directly
      if (userRole === 'admin') {
        const res = await fetch('/api/admin/approvals', {
          ...
          body: JSON.stringify({
            ...
            payload: {
              slug: activeSlug,
              label: SLUG_META[activeSlug].label,
              overrides: overrides[activeSlug] ?? {},
              full_content: newContent,
              changed_fields: Object.keys(overrides[activeSlug] ?? {}),
            },
          }),
        });
      ...
      // Manager / superadmin → save directly
  ...
  const saveLabel = () => {
    if (status === 'saving') return <><span style={{ width:14, height:14, border:'2px solid rgba(255,255,255,0.3)', borderTopColor:'white', borderRadius:'50%', display:'inline-block', animation:'spin 0.7s linear infinite' }} />Menyimpan…</>;
    if (isAdmin) return '📤 Kirim ke Manager';
  ...
  return (
    <div style={{ padding:32, fontFamily:"'DM Sans',sans-serif", maxWidth:900 }}>
      <style>{`
        ...
        .badge-anim { animation: fadeIn 0.2s ease; }
      ...
      <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:16, flexWrap:'wrap', gap:16 }}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:12 }}>
          {status === 'saved' && <span className="badge-anim" style={{ fontSize:13, color:'#10B981', fontWeight:600 }}>✓ Tersimpan!</span>}
          {status === 'pending_approval' && <span className="badge-anim" style={{ fontSize:13, color:'#F59E0B', fontWeight:600 }}>⏳ Dikirim ke Manager</span>}
          {status === 'error' && <span className="badge-anim" style={{ fontSize:13, color:'#EF4444', fontWeight:600 }}>✗ Gagal</span>}
          ...
          <button onClick={handleSave} disabled={status === 'saving'}
            style={{ padding:'10px 24px', borderRadius:10, border:'none', cursor:'pointer', background: status==='saving'?'#9CA3AF': isAdmin ? 'linear-gradient(135deg,#F59E0B,#D97706)' : 'linear-gradient(135deg,#2D3F8F,#1B2A6B)', color:'white', fontSize:14, fontWeight:700, display:'flex', alignItems:'center', gap:8 }}>
            {saveLabel()}
          </button>
        </div>
      </div>
      ...
      {/* Admin notice */}
      {isAdmin && (
        <div style={{ background:'#FFFBEB', border:'1.5px solid #FDE68A', borderRadius:12, padding:'12px 16px', marginBottom:20, display:'flex', alignItems:'center', gap:10 }}>
          <span style={{ fontSize:18 }}>ℹ️</span>
          <p style={{ fontSize:13, color:'#92400E', margin:0 }}>
            Sebagai <strong>Admin</strong>, perubahan CMS akan dikirim ke <strong>Manager</strong> untuk review dan approval sebelum online.
          </p>
        </div>
      )}
              ...
              {bgImageUrl ? (
                ...
                <div className="drop-zone" onClick={() => fileRef.current?.click()}
                  ...
                  {uploading
                    ...
                    : <div><div style={{ fontSize:32, marginBottom:8 }}>📸</div><p style={{ fontWeight:600, color:'#374151', margin:'0 0 4px' }}>Klik atau drag gambar</p><p style={{ fontSize:12, color:'#9CA3AF', margin:0 }}>JPG, PNG, WebP</p></div>
                  }
                </div>
              )}
      ...
      {activeSection === 'copy' && (
        <div style={S.card}>
          ...
          <p style={{ fontSize:12, color:'#9CA3AF', background:'#F8F9FF', padding:'8px 12px', borderRadius:8, marginBottom:20 }}>
            💡 Kosongkan field untuk menggunakan copy default. Isi hanya yang ingin di-override.
          </p>
          {[

...
export default function AdminSidebar({ role, name }: { role: Role; name: string }) {
  ...
  const navItems = [
    ...
    { href:'/admin/approvals', label:'Approvals', show:can(role,'viewApprovals'),
      icon:<svg fill="none" stroke="currentColor" viewBox="0 0 24 24" width={16} height={16}><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>,
      badge: true },
    ...
    { href:'/admin/pixels', label:'Pixel & Tracking', show:can(role,'editCMS'),
      icon:<svg fill="none" stroke="currentColor" viewBox="0 0 24 24" width={16} height={16}><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 3H5a2 2 0 00-2 2v4m6-6h10a2 2 0 012 2v4M9 3v18m0 0h10a2 2 0 002-2V9M9 21H5a2 2 0 01-2-2V9m0 0h18" /></svg> },
    { href:'/admin/seo', label:'SEO Settings', show:can(role,'editCMS'),
      icon:<svg fill="none" stroke="currentColor" viewBox="0 0 24 24" width={16} height={16}><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg> },
  ];
  ...
  return (
    <>
      <style>{`
        ...
        .crm-nav-approval{color:#2D3F8F !important;background:#EEF2FF;}
        .crm-nav-approval:hover{background:#E0E7FF !important;}
        ...
        <nav className="crm-nav">
          {navItems.filter(i => i.show).map(item => (
            <Link
              key={item.href}
              href={item.href}
              className={`crm-nav-item${item.badge ? ' crm-nav-approval' : ''}`}
            >
              {item.icon}
              <span className="flex-1">{item.label}</span>
              {item.badge && (
                <span style={{
                  fontSize: 10, fontWeight: 700, background: '#2D3F8F', color: 'white',
                  padding: '1px 6px', borderRadius: 99, letterSpacing: '0.02em'
                }}>NEW</span>
              )}
            </Link>
          ))}
